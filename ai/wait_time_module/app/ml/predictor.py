...
class DurationPredictor:
    def __init__(self, artifact_dir: str|Path|None=None):
        self.artifact_dir=Path(artifact_dir) if artifact_dir else None;self.models={}; self.baselines=DEFAULTS.copy(); self.using_fallback=True; self.load_error=None;self.prediction_count=0;self.fallback_count=0;self.last_reason_code="MODEL_FALLBACK"
        if artifact_dir:
            try:
                ...
                if metadata.get("production_policy")=="model": self.models=loaded; self.using_fallback=False;self.last_reason_code=None
    ...
    def predict_service_duration(self, features):
        ...
        if not self.models:
            self.fallback_count+=1;self.last_reason_code="MODEL_FALLBACK"
        ...
        try:
            ...
            result={q:max(0.0,float(m.predict(row)[0])) for q,m in self.models.items()};self.last_reason_code=None;return result
        except Exception as exc:
            self.fallback_count+=1;self.last_reason_code="MODEL_FALLBACK";self.load_error=f"prediction failed: {exc}"
            return {"p50":self._baseline(features),"p80":self._baseline(features)*1.25,"p90":self._baseline(features)*1.5}
    @property
    def fallback_rate(self):return self.fallback_count/self.prediction_count if self.prediction_count else 0.0
