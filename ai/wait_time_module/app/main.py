...
logger=logging.getLogger("wait_time_module")
...
def health_ready():
    required={"events","queue_tasks","resources","journey_timestamps","state_meta","audit_records"}
    try:
        with store.engine.connect() as db:db.execute(text("SELECT 1"))
        missing=sorted(required-set(inspect(store.engine).get_table_names()))
        if missing:raise RuntimeError(f"missing tables: {','.join(missing)}")
        return {"status":"ready","database":"reachable","schema":"compatible","queue_engine":"initialized","model":"loaded" if not predictor.using_fallback else "baseline_fallback"}
    except Exception as exc:raise HTTPException(503,{"status":"not_ready","reason_code":"DEPENDENCY_NOT_READY","message":str(exc)}) from exc

@app.get("/health/version")
def health_version():
    metadata={}
    try:metadata=json.loads((Path(predictor.artifact_dir)/"training_metadata.json").read_text(encoding="utf-8")) if predictor.artifact_dir else {}
    except Exception:metadata={}
    return {"service_version":app.version,"state_schema_version":"3","model_version":metadata.get("model_version"),"feature_schema_version":metadata.get("feature_schema_version"),"training_data_type":metadata.get("training_data_type"),"model_fallback_rate":round(predictor.fallback_rate,6),"clinically_validated":False}

def log_event_result(e:Event,status_code:int,outcome:str):
    logger.info(json.dumps({"event_id":e.event_id,"journey_id":e.journey_id,"task_id":e.task_id,"queue_id":e.queue_id,"estimate_version":store.version,"status_code":status_code,"outcome":outcome,"correlation_id":e.correlation_id},separators=(",",":")))
...
def event(e:Event,principal:Principal=Depends(require_scope("events.write"))):
    e.actor_id=principal.actor_id;e.actor_type=principal.actor_type
    ...
    try:accepted=apply_event(store,e)
        ...
        store.audit(event_id=e.event_id,entity_type="TASK",entity_id=e.task_id,action="ASSIGNMENT_REJECTED",previous_value=str(exc.based_on_estimate_version),new_value=str(exc.current_estimate_version),reason_code=exc.reason_code,actor_id=e.actor_id,actor_type=e.actor_type,created_at=datetime.now(TZ),correlation_id=e.correlation_id)
        log_event_result(e,409,"REQUOTE_REQUIRED")
        raise HTTPException(409,{"status":"REQUOTE_REQUIRED","current_estimate_version":exc.current_estimate_version,"based_on_estimate_version":exc.based_on_estimate_version,"reason_code":exc.reason_code,"next_action":"CALL_ROOM_OPTIONS_AGAIN","message":str(exc)}) from exc
    except ValueError as exc:
        rejected=json.dumps({"event_type":e.event_type.value,"task_id":e.task_id,"queue_id":e.queue_id},separators=(",",":"))
        store.audit(event_id=e.event_id,entity_type="EVENT",entity_id=e.task_id,action="EVENT_REJECTED",previous_value=None,new_value=rejected,reason_code="INVALID_EVENT_ORDER",actor_id=e.actor_id,actor_type=e.actor_type,created_at=datetime.now(TZ),correlation_id=e.correlation_id)
        log_event_result(e,409,"INVALID_EVENT_ORDER")
        raise HTTPException(409,str(exc)) from exc
    log_event_result(e,200,"DUPLICATE" if not accepted else "ACCEPTED")
...
def estimate_task(t:Task,now:datetime|None=None):
    now=now or datetime.now(TZ);key=(t.queue_id,store.queue_version(t.queue_id),t.task_id)
    ...
    if predictor.using_fallback or predictor.last_reason_code=="MODEL_FALLBACK":reason.append("MODEL_FALLBACK")
...
def option_for(room_id,req,now):
    if not room_exists(room_id):return {"room_id":room_id,"estimate_status":"INVALID_CANDIDATE_ROOM","estimate":None,"reason_codes":["ROOM_NOT_FOUND"],"estimate_version":store.queue_version(room_id)}
    ...
    if not rs:return {"room_id":room_id,"estimate_status":"RESOURCE_UNAVAILABLE","estimate":None,"reason_codes":["NO_ACTIVE_RESOURCE"],"estimate_version":store.queue_version(room_id)}
...
def room_options(req:RoomOptionsRequest,_=Depends(require_scope("wait.read"))):
    ...
    now=datetime.now(TZ);options=[option_for(q,req,now) for q in req.candidate_room_ids];return {"request_id":req.request_id,"patient_token":req.patient_token,"generated_at":now,"valid_until":now+timedelta(minutes=2),"estimate_version":store.version,"estimate_versions":{o["room_id"]:o["estimate_version"] for o in options},"dry_run":True,"options":options}
...
def assignment_impact(req:AssignmentImpactRequest,_=Depends(require_scope("simulation.run"))):
    ...
    return {"room_id":req.room_id,"dry_run":True,"assignment_impact":impact(req.room_id,temp,datetime.now(TZ)),"estimate_version":store.queue_version(req.room_id)}
