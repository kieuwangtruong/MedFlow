...
export type VisitStatus = 'CHECKED_IN' | 'SYMPTOM_SUBMITTED' | 'WAITING_TRIAGE' | 'ROUTED' | 'WAITING' | 'WAITING_REVIEW' | 'CALLED' | 'IN_EXAMINATION' | 'WAITING_SERVICE' | 'IN_SERVICE' | 'WAITING_RESULT' | 'RESULT_READY' | 'COMPLETED' | 'CANCELLED'
...
export interface DelayAlert { isDelayed: boolean; delayMinutes: number; elapsedWaitMinutes: number; originalEstimatedWait: number; title: string; reason: string }
export interface PatientPathway { visitId: string; visitStatus: VisitStatus; queueNumber: string; currentRoom: string; currentTaskType?: 'INITIAL_CONSULT' | 'DIAGNOSTIC_SERVICE' | 'RETURN_REVIEW'; waitingForValidatedResult?: boolean; resultValidated?: boolean; peopleAhead: number; estimatedWait: number; waitedMinutes?: number; isDelayed?: boolean; delayMinutes?: number; delayAlert?: DelayAlert | null; steps: PathwayStep[] }
export interface QueueEntry { visitId: string; queueNumber: string; orderNumber: number; patientName: string; age: number; mainSymptom: string; priority: Priority; waitedMinutes: number; status: VisitStatus; department: string; room: string; calledAt?: string; taskType: 'INITIAL_CONSULT' | 'DIAGNOSTIC_SERVICE' | 'RETURN_REVIEW'; serviceType: string; resultValidated: boolean }
export interface ValidatedResult { id: string; serviceName: string; room: string; validatedAt: string; urgency: 'NORMAL' | 'URGENT' }
