...
export interface QueueEntry { visitId: string; queueNumber: string; orderNumber: number; patientName: string; age: number; mainSymptom: string; symptomPayload?: SymptomReport | null; severityScore?: number | null; intakeSource?: string | null; priority: Priority; waitedMinutes: number; status: VisitStatus; department: string; room: string; calledAt?: string; taskType: 'INITIAL_CONSULT' | 'DIAGNOSTIC_SERVICE' | 'RETURN_REVIEW'; serviceType: string; resultValidated: boolean }
