...
import type { AIRecommendation, DoctorAssignment, PatientPathway, Priority, QueueEntry, ServiceOrder, ValidatedResult } from '../types'
...
export const doctorApi = {
  ...
  getVisit: async (visitId: string): Promise<{ queue: QueueEntry; pathway: PatientPathway; recommendation: AIRecommendation; validatedResults: ValidatedResult[] }> => USE_MOCK_API ? mockDelay({ queue: queue.find((q) => q.visitId === visitId) ?? queue[0]!, pathway, recommendation: routingRecommendation, validatedResults: [] }) : (await axiosClient.get(`/doctor/visits/${visitId}`)).data,
