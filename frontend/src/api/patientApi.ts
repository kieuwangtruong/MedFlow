...
export const patientApi = {
  ...
  kioskCheckin: async (cccd: string, fullName: string): Promise<{ visitId: string; queueNumber: string; currentRoom?: string | null; requiresSymptoms?: boolean }> => USE_MOCK_API ? mockDelay({ visitId: `VIS-${cccd.slice(-6)}`, queueNumber: '--', currentRoom: null, requiresSymptoms: true }, 650) : (await axiosClient.post('/kiosk/checkins', { cccd, fullName, examinationType: 'GENERAL', patientType: 'INSURANCE' })).data,
